...
> **Revision note (v1 → v2):** Added ReAct / plan-and-execute taxonomic labels to Architect's Reflection (Review E7). No structural changes elsewhere.
...
**Taxonomic note (plan-and-execute pattern).** What you built in this chapter is the canonical *plan-and-execute* pattern — a planner step that produces a structured plan, followed by an executor step that runs each step. It's a workflow shape because Python owns the sequence. The agent loop you'll build in Chapter 4 turns this into the closely-related *ReAct* pattern (Reason + Act in a model-driven loop). Name the patterns now; you'll use the vocabulary in code reviews.
