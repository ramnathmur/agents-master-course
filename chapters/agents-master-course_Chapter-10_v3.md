...
**Code:** `code/chapter_10_recovery.py` (v3 — `Dispatcher` routes end-to-end across three scripted scenarios; every branch fires visibly in one run).

> **Revision v2 → v3:** three scenarios fire in `main()`; brief integration (Reviews E1, E6).
...
> **Tell Ram:** "Copy `chapter_10_recovery.py` (v3). The `Dispatcher` class implements RETRY_CAP=3 and REPLAN_CAP=2 with promotion. The outer loop intercepts tool results via the `capture_result` PostToolUse hook, calls `classify_failure()`, and routes accordingly."

`main()` now runs three scenarios in sequence — each one forces a distinct dispatcher branch so you can watch the full taxonomy fire in a single execution:

1. **`scenario_transient_promotion`** — a flapping endpoint returns transient failures. RETRY_CAP exhausts, the dispatcher *promotes* the class to structural, and re-plan kicks in. This is the promotion rule from Chapter 9's calibration discipline made concrete.
2. **`scenario_structural_replan`** — a schema-shape failure trips re-plan immediately, then exhausts REPLAN_CAP and escalates. Watch the re-plan budget burn down without any retry attempts polluting the trace.
3. **`scenario_policy_escalate`** — a deny-class failure (policy block) bypasses retry and re-plan entirely and routes straight to HITL. The synthetic operator denies; the loop terminates with a structured ticket.

Each scenario emits its own `dispatcher.history` segment. Read all three. The shape of the history is the audit trail every deployable agent owes its operator.
...
Expected: classification is too permissive. After K retries the failure is no longer transient — it's structural. The dispatcher should *promote* on retry exhaustion. (That's why `v3` calls `self.decide("structural")` recursively after RETRY_CAP, and why `scenario_transient_promotion` demonstrates it end-to-end.)
...
> Generate your session brief: `python code/shared/session_brief.py --after 10`.
