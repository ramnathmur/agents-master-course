# Chapter 6 — Reason: Deliberate Planning + Tool Design  [v3]
...
**Code:** `code/chapter_06_reason.py` (v3 — exposes `main_valid()` and `main_malformed()` to demonstrate the rejection-as-teaching-signal loop).

> **Revision v2 → v3:** rejection-resubmit demo in code, ReAct label, brief integration (Reviews E1, E6, E7).
...
Run `main_valid()` first — feed the tight tool a well-formed plan for "plan a 3-day trip to Goa" and watch it accept. Then run `main_malformed()` — the same tool is fed a step with a blank `expected_outcome`. The tool rejects with a structured error, the agent reads the error, re-plans, and resubmits. That rejection-resubmit cycle is the entire point of this chapter: the schema isn't just validation, it's the feedback channel that teaches the model how to think.
...
> Generate your session brief: `python code/shared/session_brief.py --after 6`.

---

## Architect's Reflection (action checklist form)

What you've built here is the canonical *ReAct* pattern (Reason + Act): the model reasons via the plan tool, acts via execution, observes the rejection, re-reasons. The schema is what makes the "observe" step legible enough to re-reason against — a free-text error would degrade into guesswork.
