...
**Code:** `code/chapter_08_integration.py` (v3 — REAL TODO-fixing agent with all five traits, now emits a real Span trace schema to `traces.jsonl`), `code/shared/eval_harness.py` (v2 — adds trajectory + LLM-judge).
...
> **Revision v2 → v3:** real Span trace schema in code, tracing-as-architecture reflection added (Reviews E2, E6).
...
> **Tell Ram:** "Two files. `chapter_08_integration.py` (v3) is now a REAL agent: list/read tools (perceive), plan tool (reason), write tool with sandbox hook (act), iteration loop (iterate), 'all TODOs resolved' predicate (goal). `shared/eval_harness.py` (v2) scores it against 3 fixture directories (clean, one_todo, multi_todo)."

New in v3: the agent writes a real trace schema to `traces.jsonl`. Each tool invocation emits a `Span` dataclass with `span_id`, `parent_id`, `tool`, `started_at_counter`, `duration_ticks`, and `result_summary`. The file is append-only JSONL — one span per line — so you can replay any run after the fact without re-executing the agent. This is the same shape OpenTelemetry uses; you are not inventing a format, you are learning the standard one.

Predict before running: which fixture is most likely to fail and why? Then run.

After the run, **inspect `traces.jsonl`**. Read the spans top-to-bottom. Reconstruct the agent's perceive→reason→act loop from the trace alone — without looking at the agent code. If you can't, the trace is under-specified and you fix it before you ship. That exercise is the point.
...
> Generate your session brief: `python code/shared/session_brief.py --after 8`.
...
6. **Tracing as architecture** — does every tool call emit a span with (a) a unique **span ID**, (b) a **parent reference** so the call tree reconstructs, (c) a **structured event shape** (dataclass or schema — not a free-form log string), and (d) an **append-only JSONL artifact** on disk? These four properties are the contract. OpenTelemetry is the industry standard for this shape — learn that the standard exists, then know that your `traces.jsonl` in Chapter 8 is a deliberately minimal subset of it. A trace you can't replay is a trace you don't have.

## Exit Check

Take any agent and define a 5-task eval set: each task has an input, a scoring function (deterministic OR trajectory OR LLM-judge), and an expected output or trajectory. You can also explain when to add a task as a regression test vs hold it out. Separately: point to the four properties of your trace schema and show a single span from `traces.jsonl` that satisfies all four.
